...
public class VoiceProcessor : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // ------- Microphone cycling with R key -------
        if (Input.GetKeyDown(KeyCode.R))
        {
            UpdateDevices();
            if (Devices != null && Devices.Count > 0)
            {
                MicrophoneIndex = (MicrophoneIndex + 1) % Devices.Count;
                Debug.Log($"[VoiceProcessor] Switched mic index to {MicrophoneIndex}: {Devices[MicrophoneIndex]}");
                ChangeDevice(MicrophoneIndex);
            }
        }
        // ---------------------------------------------
